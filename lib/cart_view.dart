...
import 'cart.dart';
import 'package:utm_app/product_details.dart';
...
class CartPageState extends State<CartPage> {
  ...
  void _incrementQuantity(int productId) {
    setState(() {
      cart.incrementQuantity(productId); // Используйте метод incrementQuantity из класса Cart
      // Нет необходимости заново загружать futureCartProducts, если данные продукта не меняются
    });
  }

  void _decrementQuantity(int productId) {
    setState(() {
      cart.decrementQuantity(productId); // Используйте метод decrementQuantity из класса Cart
      // Нет необходимости заново загружать futureCartProducts, если данные продукта не меняются
    });
  }
                ...
                rows: cart.items.entries.map<DataRow>((entry) {
                  ...
                  final productDetail = cartProducts.firstWhere(
                    ...
                    orElse: () => ProductDetails(
                      ...
                      category: Category(id: 0, title: 'Default Category', image: 'default_image.png'), // Создаем дефолтный объект Category
                      attributes: [Attribute(id: 0, title: 'Default Attribute', values: ['Default'])], // Создаем дефолтный список атрибутов
                  ...
                  return DataRow(
                    cells: <DataCell>[
                      ...
                      DataCell(Text('\$${(productDetail.price * quantity).toStringAsFixed(2)}')), // Здесь отображается общая цена за количество
                    ],
                  );
                }).toList(),
              ),
